    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="cardImg" label="身份证照片">
          <template v-slot="scope">
            <el-image v-if="scope.row.cardImg" style="width: 50px" :src="scope.row.cardImg" :preview-src-list="[scope.row.cardImg]"></el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column label="审核" align="center" width="180">
          <template v-slot="scope">
            <el-button size="mini" type="success" plain @click="audit(scope.row, '通过')">通过</el-button>
            <el-button size="mini" type="danger" plain @click="audit(scope.row, '拒绝')">拒绝</el-button>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/volApply/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/volApply/selectPage', {
        params: {
